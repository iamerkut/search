...
class Bootstrap extends Bootstrapper
{
    ...
    private ?int $settingsCacheTimestamp = null;
    ...
    private function handleSearchRequest(): void
    {
        try {
            ...
            $minChars = max(1, (int)($settings['dsb_min_chars'] ?? 4));

            $tokens = $this->tokenizeQuery($q);
            $hasImportantToken = false;
            foreach ($tokens as $token) {
                if ($this->isImportantToken($token)) {
                    $hasImportantToken = true;
                    break;
                }
            }

            if (empty($q) || (mb_strlen($q) < $minChars && !$hasImportantToken)) {
            ...
            // Products
            $productLimit = max(1, (int)($settings['dsb_limit_products'] ?? 8));
            $primaryTokens = [];
            $secondaryTokens = [];
            foreach ($tokens as $token) {
                if ($this->isImportantToken($token)) {
            ...
            $productRows = [];
            if (($settings['dsb_enable_products'] ?? 'Y') === 'Y') {
                $this->ensureIndexes($db);
                $useFulltext = $this->isFulltextAvailable($db);

                if ($useFulltext) {
                    $matchQuery = $this->buildFulltextQuery($primaryTokens, $secondaryTokens, $q);
                    $productRows = $db->getObjects(
                        "SELECT a.kArtikel AS id,
                                a.cName AS label,
                                MATCH(a.cName, a.cSuchbegriffe, a.cArtNr) AGAINST (:match IN BOOLEAN MODE) AS score
                         FROM tartikel a
                         WHERE MATCH(a.cName, a.cSuchbegriffe, a.cArtNr) AGAINST (:match IN BOOLEAN MODE) > 0
                         ORDER BY score DESC
                         LIMIT " . $productLimit,
                        ['match' => $matchQuery]
                    );
                ...
                if (!$useFulltext || empty($productRows)) {
                    $productParams = [];
                    $primaryConditions = [];
            ...
            if (!empty($productRows)) {
                $seoMap = $this->fetchSeoForArticles($db, array_map(static fn($row) => (int)$row->id, $productRows));
                foreach ($productRows as $row) {
                    $id = (int)$row->id;
                    $label = isset($row->label) ? (string)$row->label : '';
                    $seo = $seoMap[$id] ?? null;
                    $url = !empty($seo) ? '/' . ltrim($seo, '/') : '/index.php?a=' . $id;
            ...
            $categoryLimit = max(1, (int)($settings['dsb_limit_categories'] ?? 3));
            $categoryRows = $db->getObjects(
            ...
            $categorySeo = $this->fetchSeoForCategories($db, array_map(static fn($row) => (int)$row->id, $categoryRows));
            ...
            $manufacturerLimit = max(1, (int)($settings['dsb_limit_manufacturers'] ?? 3));
            $manufacturerRows = $db->getObjects(
            ...
            $manufacturerSeo = $this->fetchSeoForManufacturers($db, array_map(static fn($row) => (int)$row->id, $manufacturerRows));
            foreach ($manufacturerRows as $r) {
                $id = (int)$r->id;
            ...
            $this->logQuery($q, $resultCount, $status);
            echo json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
            $duration = (microtime(true) - $start) * 1000;
            error_log(sprintf('[DSB] handleSearchRequest term="%s" results=%d time=%.2fms tokens=%d', $q, $resultCount, $duration, count($tokens)));
    ...
    private function getCustomerGroupId(): int
    {
        try {
            ...
            $globalSettings = Shop::getSettings([
                \CONF_GLOBAL
            ]);
            $defaultGroup = $globalSettings['global']['kundengruppe_standard'] ?? null;
            return is_numeric($defaultGroup) ? (int)$defaultGroup : 1;
        } catch (\Throwable $e) {
    ...
    {
        if ($this->settingsCache !== null && $this->settingsCacheTimestamp !== null
            && (time() - $this->settingsCacheTimestamp) < 300) {
            return $this->settingsCache;
        }

        $cacheKey = 'dsb_settings_' . $this->getPlugin()->getID();
        $cached = $this->getCache()->get($cacheKey);
        if (is_array($cached)) {
            $this->settingsCache = $cached;
            $this->settingsCacheTimestamp = time();
            return $cached;
        }
        ...
        $this->settingsCacheTimestamp = time();
        $this->getCache()->set($cacheKey, $settings, 300);
        return $settings;
    }

    private function ensureIndexes(DbInterface $db): void
    {
        ...
        try {
            $existing = $db->getObjects("SHOW INDEX FROM tartikel");
            $indexMap = [];
            foreach ($existing as $row) {
                $indexMap[(string)$row->Key_name] = true;
            }

            if (empty($indexMap['idx_dsb_name'])) {
                $db->query("ALTER TABLE tartikel ADD INDEX idx_dsb_name (cName(191))");
            }

            if (empty($indexMap['idx_dsb_artnr'])) {
                $db->query("ALTER TABLE tartikel ADD INDEX idx_dsb_artnr (cArtNr)");
            }

            if (empty($indexMap['idx_dsb_suchbegriffe'])) {
                $db->query("ALTER TABLE tartikel ADD INDEX idx_dsb_suchbegriffe (cSuchbegriffe(191))");
            }
        } catch (\Throwable $e) {
            error_log('[DSB] ensureIndexes tartikel: ' . $e->getMessage());
        }

        try {
            $existingSeo = $db->getObjects("SHOW INDEX FROM tseo WHERE Key_name = 'idx_dsb_tseo'");
            if (empty($existingSeo)) {
                $db->query("ALTER TABLE tseo ADD INDEX idx_dsb_tseo (cKey, kKey)");
            }
        } catch (\Throwable $e) {
            error_log('[DSB] ensureIndexes tseo: ' . $e->getMessage());
        }

        $this->indexesEnsured = true;
    }

    private function isFulltextAvailable(DbInterface $db): bool
    {
        ...
        try {
            $result = $db->getObjects("SHOW INDEX FROM tartikel WHERE Key_name = 'ft_artikel'");
            $this->fulltextAvailable = !empty($result);
        } catch (\Throwable $e) {
            error_log('[DSB] isFulltextAvailable check failed: ' . $e->getMessage());
    ...
    private function buildFulltextQuery(array $primaryTokens, array $secondaryTokens, string $original): string
    {
        $terms = [];
        ...
        if (empty($terms)) {
            $fallback = preg_replace('/[^\p{L}\p{N}\s]/u', ' ', $original);
    ...
    private function fetchSeoForArticles(DbInterface $db, array $ids): array
    {
        ...
        $idList = implode(',', $ids);
        $rows = $db->getObjects(
            "SELECT kKey, cSeo FROM tseo WHERE cKey = 'kArtikel' AND kKey IN ($idList)"
        );
    ...
    private function fetchSeoForCategories(DbInterface $db, array $ids): array
    {
        $ids = array_values(array_unique(array_map('intval', $ids)));
        if (empty($ids)) {
            return [];
        ...
        $idList = implode(',', $ids);
        $rows = $db->getObjects(
            "SELECT kKey, cSeo FROM tseo WHERE cKey = 'kKategorie' AND kKey IN ($idList)"
        );
    ...
    private function fetchSeoForManufacturers(DbInterface $db, array $ids): array
    {
        $ids = array_values(array_unique(array_map('intval', $ids)));
        if (empty($ids)) {
            return [];
        ...
        $idList = implode(',', $ids);
        $rows = $db->getObjects(
            "SELECT kKey, cSeo FROM tseo WHERE cKey = 'kHersteller' AND kKey IN ($idList)"
        );
